...
interface CourseEnrollmentCardProps {
  ...
  onEnroll: () => void;
  onDrop?: () => void;
  /** For shared courses: show "Start Learning" button that calls this */
  onStartLearning?: () => void;
  sourceTenantName?: string | null;
  /** For showing progress when enrolled */
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-4 sm:px-6 py-4 sm:py-5">
        <h3 className="text-lg sm:text-xl font-bold text-white">Get Started</h3>
        ...
        <div className="pt-2 space-y-3">
            ...
            <>
              ...
              {onStartLearning ? (
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-blue-600 text-white rounded-lg font-semibold text-sm hover:bg-blue-700 transition-colors"
                >
                  Start Learning
                </button>
              ) : (
                <Link
                  href="/dashboard"
                  className="block w-full text-center py-2.5 px-4 bg-blue-600 text-white rounded-lg font-semibold text-sm hover:bg-blue-700 transition-colors"
                >
                  View in My Courses
                </Link>
              )}
          ...
          {enrollmentStatus === 'not_enrolled' && (
            <button
              onClick={onEnroll}
              disabled={enrolling}
              className="w-full py-3 px-4 bg-blue-600 text-white rounded-lg font-semibold text-sm hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {enrolling ? (
                <>
