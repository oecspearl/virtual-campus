  ...
  const videoPlayer = (
    ...
    >
      ...
      >
        ...
        {/* Control Buttons */}
        <div className="flex items-center gap-1 sm:gap-2 px-3 py-2">
          {/* Play/Pause */}
          ...
          <div className="flex items-center gap-1 ml-1">
            <button onClick={toggleMute} className="p-1.5 text-white/70 hover:text-white transition-colors" aria-label={isMuted ? 'Unmute (m)' : 'Mute (m)'}>
              {isMuted || volume === 0 ? (
            ...
            <input
              ...
              className="w-16 sm:w-20 h-1 bg-white/30 rounded-full appearance-none cursor-pointer accent-white"
              aria-label="Volume"
            />
          </div>
          ...
          {captions && captions.length > 0 && (
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <button
                ...
                className={`p-1.5 transition-colors ${activeCaptionIdx !== null ? 'text-blue-300' : 'text-white/70 hover:text-white'}`}
              ...
              {showCaptionMenu && (
                <div className="absolute bottom-full right-0 mb-2 bg-gray-900/95 backdrop-blur rounded-lg shadow-lg py-1 min-w-[120px] z-10">
                  <button
          ...
          {/* Fullscreen */}
          <button onClick={toggleFullscreen} className="p-1.5 text-white/70 hover:text-white transition-colors" aria-label={isFullscreen ? 'Exit fullscreen (f)' : 'Fullscreen (f)'}>
            {isFullscreen ? (
