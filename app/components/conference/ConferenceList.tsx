...
import { Users, Video, Layout } from 'lucide-react';
...
export default function ConferenceList({
  ...
  // Modal states for attendance, recording, and whiteboard management
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <LoadingIndicator variant="dots" size="sm" text="Loading conferences..." />
      </div>
    );
  }
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {conferences.map((conference) => (
          <div
            key={conference.id}
            className="bg-white border border-gray-200 rounded-lg p-4 sm:p-5 hover:shadow-lg hover:border-blue-200 transition-all duration-200 overflow-hidden"
          >
            <div className="space-y-3">
              {/* Conference info */}
              <div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* Whiteboard button — visible for live/scheduled (anyone) and ended (instructor) */}
                {(canJoin(conference) || (isInstructor && conference.status === 'ended')) && (
                  <button
                    onClick={() => setWhiteboardConference(conference)}
                    className="inline-flex items-center px-3 py-2 bg-violet-50 hover:bg-violet-100 text-violet-700 text-sm font-medium rounded-lg border border-violet-200 transition-all duration-200"
    ...
    {/* Whiteboard Modal */}
    {whiteboardConference && (
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full max-w-5xl h-[85vh] flex flex-col shadow-lg overflow-hidden">
          {/* Modal header */}
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-violet-50 to-purple-50 border-b border-gray-200 flex-shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-gray-900 truncate">
                  Whiteboards — {whiteboardConference.title}
                </h3>
                <p className="text-xs text-gray-500">
                  {whiteboardConference.status === 'ended' ? 'View saved boards' : 'Collaborate in real-time'}
                </p>
              </div>
            </div>
            <button
              onClick={() => setWhiteboardConference(null)}
          ...
          <div className="flex-1 min-h-0">
            <ConferenceWhiteboardPanel
              conferenceId={whiteboardConference.id}
              isHost={isInstructor}
              isVisible={true}
              onClose={() => setWhiteboardConference(null)}
            />
          </div>
        </div>
      </div>
