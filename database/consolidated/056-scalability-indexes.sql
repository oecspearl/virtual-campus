...
--
-- LOCKING NOTE: Plain CREATE INDEX takes a SHARE lock on the table during
-- the build. For the table sizes in this app today (well under a million
-- rows per table), each index builds in well under a second, so writes
-- are blocked only briefly. Once a table approaches multi-million rows
-- the future indexing strategy should switch to CREATE INDEX CONCURRENTLY
-- (which has to be run outside any transaction block — Supabase SQL
-- editor cannot do that, so use psql or `supabase db push` instead).
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_quiz_attempts_tenant_student
...
CREATE INDEX IF NOT EXISTS idx_student_activity_log_tenant_student
  ON student_activity_log(tenant_id, student_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_student_activity_log_tenant_course
  ON student_activity_log(tenant_id, course_id, created_at DESC);

-- course_announcements: timeline reads
CREATE INDEX IF NOT EXISTS idx_course_announcements_tenant_course_date
...
CREATE INDEX IF NOT EXISTS idx_discussion_replies_tenant_discussion
  ON discussion_replies(tenant_id, discussion_id);

CREATE INDEX IF NOT EXISTS idx_lesson_discussion_replies_tenant_discussion
  ON lesson_discussion_replies(tenant_id, discussion_id);

-- Foreign keys with no covering index (slow deletes / reverse lookups)
CREATE INDEX IF NOT EXISTS idx_student_chat_members_user
  ON student_chat_members(user_id);

CREATE INDEX IF NOT EXISTS idx_student_chat_rooms_created_by
  ON student_chat_rooms(created_by);

CREATE INDEX IF NOT EXISTS idx_peer_review_assignments_reviewer
