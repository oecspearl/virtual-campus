...
## Root Cause
The original fix assumed the wrong relationship chain. In the actual database schema (without subjects):
...
- Modified the query to follow the correct relationship chain: `quiz -> lesson -> course`
...
2. Check if the lesson has a `course_id`
3. Use the debug endpoint to see the full relationship chain
...
Quiz -> Lesson -> Course
  ...
  |       |        +-- course_id
  |       +-- course_id
  +-- course_id (optional, for direct association)
```
