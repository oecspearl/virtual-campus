...
// DELETE /api/lesson-discussions/[id]/replies/[replyId] - Delete a lesson discussion reply
export async function DELETE(
  ...
  try {
    ...
    if (deleteError) {
      console.error('Error deleting lesson discussion reply:', deleteError);
      return NextResponse.json({ error: "Failed to delete reply" }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Delete lesson discussion reply error:', error);
