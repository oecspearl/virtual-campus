                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (caption && caption.caption_url) {
                        // Normalize language code (Whisper returns "english", we need "en")
                        const langCode = caption.language?.length > 3
                          ? ({ english: 'en', spanish: 'es', french: 'fr', german: 'de', portuguese: 'pt', italian: 'it', dutch: 'nl', russian: 'ru', chinese: 'zh', japanese: 'ja', korean: 'ko', arabic: 'ar', hindi: 'hi' } as Record<string, string>)[caption.language?.toLowerCase()] || caption.language
                          : caption.language || 'en';

                        const existingCaptions = (vd.captions || []).filter(
                          (c: any) => c.srclang !== langCode
                        );
                        const newCaption = {
