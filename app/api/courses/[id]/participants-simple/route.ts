...
export async function GET(
  ...
  console.log('=== SIMPLE PARTICIPANTS API ===');
  ...
  try {
    console.log('Starting simple participants fetch for course:', courseId);
    ...
    console.log('Test query result:', { 
      hasData: !!testQuery && testQuery.length > 0,
      error: testError,
    ...
    // Now try the full query with only basic columns first
    ...
    console.log(`Simple participants fetch completed in ${responseTime}ms for course ${courseId}`);
    ...
    const responseTime = Date.now() - startTime;
    console.error('Simple Participants API Error:', {
